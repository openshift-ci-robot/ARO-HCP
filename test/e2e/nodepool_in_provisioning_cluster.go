...
var _ = Describe("Customer", func() {
	...
	It("should not be able to deploy a node pool in a cluster that is in provisioning state",
		...
		func(ctx context.Context) {
			...
			_, err = framework.CreateBicepTemplate(ctx,
				tc.GetARMResourcesClientFactoryOrDie(ctx).NewDeploymentsClient(),
				*resourceGroup.Name,
				"cluster",
			...
			// Check that the cluster is in provisioning state
			result, err := framework.GetHCPCluster(ctx,
				tc.Get20240610ClientFactoryOrDie(ctx).NewHcpOpenShiftClustersClient(),
				*resourceGroup.Name,
				customerClusterName)
			Expect(err).NotTo(HaveOccurred())
			Expect(*result.Properties.ProvisioningState).To(Equal(hcpsdk20240610preview.ProvisioningStateProvisioning))

			By("creating the node pool when the cluster has not been completely deployed")
			...
			// TODO: Assert specific error message
		})
})
