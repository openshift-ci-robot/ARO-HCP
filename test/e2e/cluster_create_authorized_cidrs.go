	...
	Context("Cluster Creation", func() {
		It("should be able to create a HCP cluster with valid authorized CIDRs",
			...
			func(ctx context.Context) {
				const clusterName = "with-authorized-cidrs-cl"
				...
				By("deploying bicep template with authorized CIDRs")
				_, err = framework.CreateBicepTemplateAndWait(ctx,
					tc.GetARMResourcesClientFactoryOrDie(ctx).NewDeploymentsClient(),
					*resourceGroup.Name,
					"aro-hcp-with-authorized-cidrs",
					framework.Must(TestArtifactsFS.ReadFile("test-artifacts/generated-test-artifacts/with-authorized-cidrs.json")),
